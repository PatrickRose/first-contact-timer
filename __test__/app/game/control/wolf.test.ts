...
type RouteModule =
    typeof import("../../../../src/app/game/[id]/control/api/wolf/route");
...
beforeAll(async () => {
    ({ POST } =
        await import("../../../../src/app/game/[id]/control/api/wolf/route"));
...
function wolfComponent(): Game["components"][0] {
    return {
...
describe("POST /game/[id]/control/api/wolf", () => {
    test("returns a 400 for an invalid body", async () => {
        const response = await POST(
            makeRequest({ newStatus: "yes" }),
            makeProps(),
        );
    ...
    test("returns a 500 when the game has no wolf attack component", async () => {
        const repo = makeFakeGameRepo(makeActiveGame());
        getGameRepo.mockReturnValue(MakeRight(repo));
        ...
        expect(await response.json()).toEqual({
            error: "No wolf component for game test-game",
        });
    });

    test("starts a wolf attack", async () => {
        const repo = makeFakeGameRepo(
            makeActiveGame({ components: [wolfComponent()] }),
        );
        getGameRepo.mockReturnValue(MakeRight(repo));
