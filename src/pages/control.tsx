  ...
  return (
    <div className={`flex mx-1`}>
      ...
      {updatingTo === null ? (
        ...
        <div className="flex-1">Updating to defcon level {updatingTo}...</div>
      )}
    </div>
  );
}
    ...
    if (this.state.errorMessage !== undefined) {
      return (
        <div className="bg-red-600 text-white mt-4 p-4">
          <p>
            There was an issue running the Control command - wait and try again.
            If it persists, grab Paddy.
          </p>
          <p>
            Message was:{" "}
            <code className="bg-black px-2">{this.state.errorMessage}</code>
          </p>
        </div>
      );
    }
