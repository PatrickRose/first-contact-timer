...
export default function BreakingNews({content}: BreakingNewsProps) {
    const values = Object.values(content).filter(val => val !== null) as string[];
    ...
    const getNextBreakingNews = (lastBreakingNews?: string): string | null => {
        const key = values.indexOf(lastBreakingNews === undefined ? '' : lastBreakingNews);
    ...
    >
        ...
        >
            ...
            >
                <div>
                    {activeValue.value?.replace('\n\n', '\n').split('\n').map((val, key) => {
                        return <p className="py-2" key={key}>{val}</p>
                    })}
                </div>
            </Transition.Child>
        </Transition>
