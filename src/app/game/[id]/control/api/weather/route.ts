...
import {ControlAPIDecode, DefconAPIBodyDecode, SetWeatherStatusDecode} from "../../../../../../types/io-ts-def";
...
export async function POST(request: NextRequest, {params}: { params: { id: string } }): Promise<NextResponse<ApiResponse|{error: string}>> {
    ...
    const newGame = await gameRepo.right.runControlAction(
        ...
        (game) => {
            ...
            const defconComponent = newGame.components.find(val => val.componentType == "Weather");
            ...
            if (!newGame.active) {
                const frozenComponent = newGame.frozenTurn.components.find(val => val.componentType == "Weather");
