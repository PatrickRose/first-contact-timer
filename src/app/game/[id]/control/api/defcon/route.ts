...
import {ControlAPIDecode, DefconAPIBodyDecode} from "../../../../../../types/io-ts-def";
...
export async function POST(request: NextRequest, {params}: { params: { id: string } }): Promise<NextResponse<ApiResponse|{error: string}>> {
    ...
    const newGame = await gameRepo.right.runControlAction(
        ...
        (game) => {
            ...
            const defconComponent = newGame.components.find(val => val.componentType == "Defcon");
            ...
            if (!country) {
                return MakeLeft(`Defcon component does not include ${body.stateName}`);
            }

            defconComponent.countries[body.stateName].status = body.newStatus;

            if (!newGame.active) {
                const frozenComponent = newGame.frozenTurn.components.find(val => val.componentType == "Defcon");

                if (frozenComponent?.componentType == "Defcon") {
                    frozenComponent.countries[body.stateName].status = body.newStatus;
                }
            }
